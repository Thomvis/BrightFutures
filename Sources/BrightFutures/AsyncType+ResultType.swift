...
public extension AsyncType where Value: ResultProtocol {
    ...
    /// Returns self
    @discardableResult
    public func onSuccess(_ context: @escaping ExecutionContext = defaultContext(), callback: @escaping (Value.Value) -> Void) -> Self {
    ...
    /// Returns self
    @discardableResult
    public func onFailure(_ context: @escaping ExecutionContext = defaultContext(), callback: @escaping (Value.Error) -> Void) -> Self {
        self.onComplete(context) { result in
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recover(context c: @escaping ExecutionContext = defaultContext(), task: @escaping (Value.Error) -> Value.Value) -> Future<Value.Value, NoError> {
    ...
    /// The closure is executed on the given context. If no context is given, the behavior is defined by the default threading model (see README.md)
    public func recoverWith<E1>(context c: @escaping ExecutionContext = defaultContext(), task: @escaping (Value.Error) -> Future<Value.Value, E1>) -> Future<Value.Value, E1> {
